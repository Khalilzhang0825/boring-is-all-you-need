...
- Before modifying files, read the closest repository `AGENTS.md` and the relevant rule under `%STEADYAGENT_HOME%\rules`.
...
SteadyAgent 2 targets Codex Desktop. Managed hooks provide one `SessionStart`, two `PreToolUse` guards, and one `PreCompact` reminder. Hooks reduce mistakes but are not a complete security sandbox. Do not change the user's model or reasoning settings.
