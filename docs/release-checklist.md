...
- Confirm local and remote `v2.0.1` tags are either absent or resolve exactly to the reviewed `origin/main` commit, every native Git command is exit-checked, and an existing exact remote tag is preserved without a write.
...
- Before publishing the captured draft, require GitHub Live evidence that immutable releases are enabled and an exact active tag ruleset with no bypass actors blocks update and deletion of `refs/tags/v2.0.1`; retain the pre/post-publication guard and ref readbacks.
