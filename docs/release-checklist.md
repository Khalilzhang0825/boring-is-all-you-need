...
Use this checklist before publishing the Boring Is All You Need v3.0.1 tag or GitHub release.
...
- Confirm local and remote `v3.0.1` tags are either absent or resolve exactly to the reviewed `origin/main` commit, every native Git command is exit-checked, and an existing exact remote tag is preserved without a write.
...
- Before publishing the captured draft, require GitHub Live evidence that immutable releases are enabled and an exact active tag ruleset with no bypass actors blocks update and deletion of `refs/tags/v3.0.1`; retain the pre/post-publication guard and ref readbacks.
- Confirm `git diff --check f80c05c4b79e069ee3a35db3c09a8f870bca0b59...HEAD` has no whitespace errors across the complete v3.0.1 release change range.
