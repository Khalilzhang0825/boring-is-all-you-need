...
- Confirm local and remote `v2.0.0` tags are either absent or resolve exactly to the reviewed `origin/main` commit, every native Git command is exit-checked, and an existing exact remote tag is preserved without a write.
...
- Confirm production Apply and rollback reject an elevated token before migration writes, never request UAC, and contain no protected-recovery production entrypoint.
...
- Confirm all examples use ordinary non-elevated commands and clearly report administrator-locked managed configuration as unsupported. Do not treat a receipt hash as authentication.
...
- Before publishing the captured draft, require GitHub Live evidence that immutable releases are enabled and an exact active tag ruleset with no bypass actors blocks update and deletion of `refs/tags/v2.0.0`; retain the pre/post-publication guard and ref readbacks.
...
- Known limits: Windows-first scripts, Codex managed hooks require restart-time Live verification, automated validation does not install into a real global Codex configuration, same-user guardrails are cooperative rather than an adversarial sandbox, and administrator-locked managed configuration is unsupported.
