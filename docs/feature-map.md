...
| Receipt rollback | `tools/rollback.ps1` | `$HOME\.steadyagent\tools\` or same verified release package during early-crash recovery | durable `applying`/`applied` receipt, exact mixed-state, snapshot, script-identity, elevated-token compatibility and zero-write dry-run tests. |
