...
the protected `v2.0.1` tag, and repository Actions allowed to write contents,
...
The release artifact must be built by `.github/workflows/release.yml` from the exact `v2.0.1` tag. Do not upload a locally assembled replacement archive.
...
```text
Tag: v2.0.1
Title: Boring Is All You Need v2.0.1
...
Expand-Archive .\boring-is-all-you-need-v2.0.1.zip .\release-check
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\release-check\boring-is-all-you-need-v2.0.1\tools\validate-release-archive.ps1
```
...
gh run download $RunId -R $Repository -n boring-is-all-you-need-v2.0.1-release-bundle -D $ExpectedRoot
...
if ($CapturedReleaseId -le 0 -or -not [bool]$CapturedDraft.draft -or [bool]$CapturedDraft.prerelease -or
    [string]$CapturedDraft.tag_name -cne $Tag -or [string]$CapturedDraft.name -cne "Boring Is All You Need v2.0.1" -or
...
$ExpectedBody = $ReleaseNotes + "`n`n## Verified provenance`n`nReviewed commit: $ReviewedSha`nSource ref: refs/tags/v2.0.1`n"
...
function Assert-ExactReleaseDraft {
  ...
  if ([long]$State.id -ne $ReleaseId -or -not [bool]$State.draft -or [bool]$State.prerelease -or
      [string]$State.tag_name -cne $Tag -or [string]$State.name -cne "Boring Is All You Need v2.0.1" -or
      [string]$State.body -cne $ExpectedBody -or ($assetNames -join "|") -cne
      "boring-is-all-you-need-v2.0.1.provenance.json|boring-is-all-you-need-v2.0.1.zip|boring-is-all-you-need-v2.0.1.zip.sha256") {
...
if ($LiveTagSha.Trim() -ne $ReviewedSha -or $LiveMainSha.Trim() -ne $ReviewedSha) {
  throw "Live v2.0.1 or main moved away from the reviewed commit."
}
...
foreach ($state in @($Published, $Readback)) {
  ...
  if ([long]$state.id -ne $CapturedReleaseId -or [bool]$state.draft -or [bool]$state.prerelease -or
      ...
      [string]$state.tag_name -cne $Tag -or [string]$state.name -cne "Boring Is All You Need v2.0.1" -or
      [string]$state.body -cne $ExpectedBody -or ($assetNames -join "|") -cne
      "boring-is-all-you-need-v2.0.1.provenance.json|boring-is-all-you-need-v2.0.1.zip|boring-is-all-you-need-v2.0.1.zip.sha256" -or
