...
the protected `v3.0.1` tag, and repository Actions allowed to write contents,
...
The release artifact must be built by `.github/workflows/release.yml` from the exact `v3.0.1` tag. Do not upload a locally assembled replacement archive.
...
if ($LiveTagSha.Trim() -ne $ReviewedSha -or $LiveMainSha.Trim() -ne $ReviewedSha) {
  throw "Live v3.0.1 or main moved away from the reviewed commit."
}
