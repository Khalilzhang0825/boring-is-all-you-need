...
发布 Boring Is All You Need v3.0.0 tag 或 GitHub release 前，用这份清单做最后验收。
...
- 确认本地与远端 `v3.0.0` tag 均只能处于不存在或精确指向已审查 `origin/main` commit 两种状态，每条 native Git 命令都检查退出码，远端已存在 exact tag 时不执行写入。
...
- 发布 captured draft 前，必须保留 GitHub Live 证据：immutable releases 已启用，且存在精确匹配 `refs/tags/v3.0.0`、active、无 bypass actor、阻止 update/deletion 的 tag ruleset；同时保留发布前后 guard 与 ref 回读。
- 确认完整 v3.0.0 发布变更范围运行 `git diff --check f80c05c4b79e069ee3a35db3c09a8f870bca0b59...HEAD` 没有 whitespace errors。
