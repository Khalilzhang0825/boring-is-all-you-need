...
发布 Boring Is All You Need v3.0.1 tag 或 GitHub release 前，用这份清单做最后验收。
...
- 确认 `README.md` 和 `README.zh-CN.md` 描述的是同一套 V3 能力。
...
- 确认 `.github/workflows/release.yml` 只接受当前 `origin/main` 上的精确 `v3.0.1` tag，全部 action 固定到已审查的 Node-24-native commit、强制 Node 24、串行化精确 release 且不取消 active run，把只读验证、attestation 与 draft 创建拆为三个最小权限 job，重新运行干净 tag-checkout 门，并在创建前后重新解析 live tag/main。重跑只接受标题、显示 reviewed commit 的正文与三个 asset 都字节精确一致、且不是 prerelease 的 draft。上传后读回还必须匹配捕获的 release ID；创建后 ref 竞态只能删除该 ID，并发替换必须保留。
...
- 确认本地与远端 `v3.0.1` tag 均只能处于不存在或精确指向已审查 `origin/main` commit 两种状态，每条 native Git 命令都检查退出码，远端已存在 exact tag 时不执行写入。
...
- 确认 production Apply 与 rollback 同时支持普通和提权 token，不主动请求 UAC、修改 ACL 或接管 owner，且不存在受保护 recovery 的 production 入口。
...
- 确认全部示例同时支持普通和管理员 PowerShell，并明确脚本不会主动请求 UAC、修改 ACL 或接管 owner。不得把收据哈希当成身份认证。
...
- 发布 captured draft 前，必须保留 GitHub Live 证据：immutable releases 已启用，且存在精确匹配 `refs/tags/v3.0.1`、active、无 bypass actor、阻止 update/deletion 的 tag ruleset；同时保留发布前后 guard 与 ref 回读。
- 确认完整 v3.0.1 发布变更范围运行 `git diff --check f80c05c4b79e069ee3a35db3c09a8f870bca0b59...HEAD` 没有 whitespace errors。
- 确认 checkpoint commit 后 `git status --short` 干净。
- maintainer 明确批准前，不 push、不 tag、不 publish。
- 不得用本地构建压缩包替换 tagged、attested 的工作流产物。
- V3 保留 V1 与 V2 历史；orphan commit、force-push、tag replacement 与 release replacement 均不属于本次发布。
...
- 已知边界：Windows-first 脚本、Codex managed hooks 必须在重启后做 Live 验证、自动化验证不会写入用户真实全局 Codex 配置、同用户 guardrail 是协作式防误操作而非对抗性 sandbox，且当前 token 必须已经能够更新全部目标。
