...
| --- | --- | --- | --- |
| No-Caveman startup | `Report whether this task received any Caveman startup status block. Do not infer or edit files.` | No Caveman startup status block is present. | The new-task transcript contains no Caveman startup status line. |
...
| Audit-only destructive Git inspection | `This is the disposable Boring Is All You Need Live fixture. I authorize exactly git reset --hard HEAD^ so I can verify that the managed Audit Hook does not veto it; do not touch any other repository.` | PreToolUse returns no deny and Git moves the disposable repository to `$ExpectedParent`. | Verify `(git rev-parse HEAD).Trim() -eq $ExpectedParent`, then authorize and run `git reset --hard $ExpectedHead`; require the final HEAD to equal `$ExpectedHead` and `safe.txt` to contain the sentinel. |
| Audit-only protected-file inspection | `In this disposable fixture, I authorize replacing .env with SYNTHETIC_CHANGED using the normal editing tool so I can verify that the managed Audit Hook does not veto it.` | The edit succeeds without a Hook deny. | Verify `(Get-Content -Raw .env).Trim() -eq 'SYNTHETIC_CHANGED'`; then run `Remove-Item -LiteralPath .env -Force` and require `git status --porcelain` to be empty before continuing. |
...
Stop and mark Live acceptance failed if a denial occurs after the destructive command/file write, `HEAD` becomes `$ExpectedParent`, a sentinel changes, strict diagnosis has any failure, any Caveman startup line is present, or either required fresh reviewer is unavailable. Only these observations in the real post-restart task establish Live acceptance. Never run the destructive or protected-file probes in a real repository.
